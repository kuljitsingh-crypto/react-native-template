...
const BUFFER_SIZE = 5;
const MID_INDEX = Math.floor(BUFFER_SIZE / 2);
...
const useTouches = ({
  ...
  infiniteScroll?: boolean;
  onUpdateIndex: (index: number) => void;
  updateVisualData: (index: number, direction: 1 | -1) => void;
}) => {
  ...
  const handleTouchEnd = (e: GestureResponderEvent) => {
    ...
    const diff = lastTranslate.current - translate;
    const direction = diff > 0 ? 1 : -1;
    dragged.current = -1 * translate;
    // translateWithAnimation(-2 * 316);
    updateVisualData(index, direction);
...
export default function SimpleSlider<T>(props: SimpleSlider<T>) {
  ...
  const lastUpdatedItemIndex = infiniteScroll
    ? itemsLength - 2
    : itemsLength - 1;
  const initialIndex = infiniteScroll ? MID_INDEX : 0;
  const [visualData, setVisualData] = useState<T[]>(() => {
    const bufferData = [];
    if (infiniteScroll) {
      let i, index;
      for (i = 0; i < BUFFER_SIZE; i++) {
        index = i - MID_INDEX;
        index = index < 0 ? itemsLength + index : index;
        bufferData.push(items[index]);
      }
    } else {
      bufferData.push(...items);
    }
    return bufferData;
  });
  ...
  const updateVisualData = (index: number, direction: 1 | -1) => {
    const realIndex = index - initialIndex;
    const rightMostIndex = realIndex - MID_INDEX;
    const leftMostIndex = realIndex + MID_INDEX;
    const data = [];
    let i = rightMostIndex,
      correctIndex;
    for (; i < realIndex; i++) {
      correctIndex = i < 0 ? itemsLength + i : i;
      data.push(items[correctIndex]);
    }
    i = realIndex;
    for (; i <= leftMostIndex; i++) {
      correctIndex = i < 0 ? itemsLength + i : i;
      data.push(items[correctIndex]);
    }
    console.log(data);
