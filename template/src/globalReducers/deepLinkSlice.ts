...
const initialState: DeepLinkStateType = {
  deepLinkStatus: config.fetchStatus.idle,
  redirectPath: screenNames.home,
  redirectPathParams: undefined,
  deepLinkOriginType: config.deepLinkOriginType.none,
...
const updateProcessStatusAndRedirectPath = (
  ...
  dispatch(updateDeepLinkStatus(config.fetchStatus.succeeded));
};
  ...
  switch (pathName) {
    default: {
      updateProcessStatusAndRedirectPath(dispatch, {
        ...
        origin: config.deepLinkOriginType.none,
        shouldRedirect: false,
      });
    }
  }
